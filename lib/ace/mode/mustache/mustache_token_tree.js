...
(function() {    
    ...
    this.getInputNode = function(row, column) {
        var tokens = this.tokenizer.getLineTokens(this.doc.getLine(row)).tokens;
        var c = 0;
        for(var i = 0 ; i < tokens.length ; i++) {
            c += tokens[i].value.length;
            if(c >= column) {
                return new MustacheNode(tokens[i]);
            }
        }
        return new MustacheNode(tokens[tokens.length - 1]);
    }
    ...
    this.getParent = function(row, column) {
        var inputNode = this.getInputNode(row, column);
    ...
    this._clearDirty = function(startRange, endRange) {
        var oldNodeList, effectives = [], removed = [];
        for(var i = startRange.row ; i <= endRange.row ; i++) {
            oldNodeList = this.list.get(i);
            if(oldNodeList) {
                for(var j = 0 ; j < oldNodeList.size() ; j++) {
                    removed.push(oldNodeList.get(j));
                    var effectives = this._refine(effectives, oldNodeList.get(j));
                }
                this.list.removeAt(i);
            }
        }
        var depth = 0, index = 0, path = [];
        for(var i = 0 ; i < effectives.length ; i++) {
            var n = effectives[i].getParent();
            while(n) {
                depth++;
                n = n.getParent();
            }
            var siblings = effectives[i].getParent().getChildren();
            for(var j = 0 ; j < siblings.length ; j++) {
                if(siblings[j] === effectives[i]) {
                    index = j;
                }
            }
            path.push({depth : depth, index : index, token : effectives[i].getNode().token});
            depth = index = 0;
        }
        for(var i = 0 ; i < removed.length ; i++) {
            removed[i].getParent().removeChild(removed[i]);
        }
        this._emit("changed", {event : "remove", object : path});
    }
    ...
    this.partialParse = function(startRange, endRange) {
        ...
        var nodeList, tokens, token, c, astNode, peak;

        var parentAstNode = new InsertAwareMustacheAST(this.getParent(startRange.row, startRange.column),
            this.getNextSibling(startRange.row, startRange.column));
        ...
        for(var i = startRange.row ; i <= endRange.row ; i++) {
            ...
            for(var j = 0 ; j < tokens.length ; j++) {
                ...
                if(!peak) {
                    peak = new InsertAwareMustacheAST(this.getParent(i, c), this.getNextSibling(i, c));
                    stack.push(peak);
                }
                ...
                if(astNode.getNode().isComment()) {
                    ...
                    if(this._endBlock(astNode, peak, stack)) {
//                        var len = this.doc.getLength(), root = this.documetAstNode;
//                        var iter = new MustacheNodeGroupIterator(oldList,
//                            {row : i, column : c}, {row : len - 1, column : this.doc.getLine(len - 1).length});
//                        iter.iterate(function(_astNode) {
//                            if(astNode.getParent() !== root && astNode !== _astNode &&
//                                _astNode.getParent() === (peak.astNode ? peak.astNode : peak)) {
//                                _astNode.setParent(peak.getParent());
//                                peak.getParent().appendChild(_astNode);
//                            }
//                        });
                    }
