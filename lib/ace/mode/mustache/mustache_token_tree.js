...
(function() {    
    ...
    this.partialParse = function(startRange, endRange) {
        ...
        //find start node;
        for(var i = 0 ; group && i < group.size() ; i++) {
            node = group.get(i);
        ...
        if(!node) {
            new Error("NotFound start {" + startRange.row + ", " + startRange.column + "} ~ end {" + endRange.row + ", " + endRange.column + "}");
        }
        ...
        iter.iterate(function(node, row, column) {
    ...
    this.change = function(range) {
        ...
        var removedList = this.partialParse(start, {
            row : Math.min(end.row, this.list.size() > 0 ? this.list.size() - 1 : this.list.size()), 
            column : end.column 
        });
        ...
        var newList = this.partialParse(start, end);
